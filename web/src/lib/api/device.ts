...
/**
 * JSON snapshot mirror of `server/src/session.rs::SessionState`. Optional
 * fields are populated only when meaningful for the active source.
 */
export interface SessionState {
  id: string;
  source_kind: 'sine' | 'file' | 'soapy';
  sample_rate_hz: number;
  center_freq_hz: number;
  fft_size: number;
  fft_rate_hz: number;
...
/**
 * Fetch the current settings snapshot for an open session. Returns `null`
 * on 404 — the session was evicted between calls and the caller should
 * tear down its local state.
 */
export async function fetchSessionState(sessionId: string): Promise<SessionState | null> {
  const r = await fetch(`/api/device/${encodeURIComponent(sessionId)}/state`);
...
/**
 * Mirror of `server/src/session.rs::PatchRequest`. All fields optional;
 * only set fields are applied. The server silently no-ops fields that
 * the active source doesn't support (e.g. gain on a sine source).
 */
...
/** PATCH /api/device/:id/settings — returns the post-apply snapshot. */
export async function patchSettings(
  sessionId: string,
  req: PatchSettingsRequest,
): Promise<SessionState | null> {
  const r = await fetch(`/api/device/${encodeURIComponent(sessionId)}/settings`, {
    method: 'PATCH',
    headers: { 'content-type': 'application/json' },
    body: JSON.stringify(req),
  });
  if (r.status === 404) return null;
  if (!r.ok) throw new ApiError(r.status, `patch failed (${r.status})`);
  return (await r.json()) as SessionState;
}
