...
/**
 * Axes for whatever stream feeds the `ui:fft` sink, accounting for any
 * channelizer in the chain. The wbfm preset pulls a 240 kHz channel
 * out of a 2.4 MHz source via a Channelizer with `freq_shift_hz` +
 * `factor` + `input_rate_hz`; the FFT then runs on that narrowed
 * band, so the spectrum/waterfall must be labelled with the
 * channelizer's *output* span and the (source + shift) centre — not
 * the raw source axes.
 *
 * Heuristic: if the composed preset exposes a block with all three
 * channelizer params, use them. Otherwise fall back to the source
 * axes (bareband presets where the FFT runs straight on the source).
 *
 * Returning the same shape as `currentAxes` keeps the renderer code
 * uniform — viz components don't need to know whether a channelizer
 * is in play.
 */
export function fftAxes(store = pipeline): PipelineAxes | null {
  const src = currentAxes(store);
  if (!src) return null;
  const chan = Object.values(store.blocks).find((b) => {
    const keys = b.spec.params.map((p) => p.key);
    return (
      keys.includes('freq_shift_hz') && keys.includes('factor') && keys.includes('input_rate_hz')
    );
  });
  if (!chan) return src;
  const v = (chan.values ?? {}) as Record<string, unknown>;
  const shift = typeof v.freq_shift_hz === 'number' ? v.freq_shift_hz : 0;
  const factor = typeof v.factor === 'number' && v.factor > 0 ? v.factor : 1;
  const inputRate = typeof v.input_rate_hz === 'number' ? v.input_rate_hz : src.sample_rate_hz;
  return {
    center_freq_hz: src.center_freq_hz + shift,
    sample_rate_hz: inputRate / factor,
  };
}
