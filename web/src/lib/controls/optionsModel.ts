...
// Single source of truth for the per-driver IF-filter ladders — Rust
// (`sdr-tables/src/lib.rs`, the `ferrite-sdr-tables` crate) owns the table;
// this file is generated by `cargo run -p ferrite-ctl -- gen-tables` and the
// Rust CI test fails if it drifts. Do not hand-edit. Keyed by lowercased
// SoapySDR driver key; a missing driver has no ladder.
import IF_FILTER_LADDERS from './if-filter-ladders.generated.json';
...
/**
 * Bandwidth choices for the advanced panel. When the driver has an
 * IF-filter ladder (the Rust-owned single source of truth), use it —
 * that's the authoritative list of filters the hardware actually has.
 * Otherwise fall back to the device probe. Sorted ascending, no dups.
 */
export function fullBandwidthChoices(caps: DeviceCapabilities): number[] {
  const ch = firstChannel(caps);
  if (!ch) return [];
  const ladder = ladderFor(caps.driver_key);
 ...
 * SoapySDR driver key — Soapy is inconsistent about casing. `undefined`
 * when the driver has no ladder (RTL-SDR, …): callers fall back to the
 * device probe / skip `set_bandwidth`.
 */
