...
impl FerriteServer {
    ...
    #[tool(
        description = "PATCH a parameter delta onto one block in the running pipeline. `block` is the block id (e.g. `src`, `chan`, `demod`, `audio_nr`); `params` is a JSON object of key→new_value pairs. Live-tunable keys hot-apply (no rebuild); others rebuild the block. The src block aliases to PATCH /api/source — its settable keys include gain_db, agc, antenna, bandwidth_hz, dc_offset_correction, and the driver-specific `settings` map (discover via `source_capabilities`). Same surface the UI's <BlockParams> component uses."
    )]
