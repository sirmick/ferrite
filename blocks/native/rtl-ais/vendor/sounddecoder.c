...
/* Ferrite-lib: `runSoundDecoder` was the file-/stdin-driven main loop;
 * unused in library mode where samples arrive synchronously through
 * `run_mem_decoder` from the runtime tick loop. Wrapped in `#if 0` so
 * upstream-syncs still diff cleanly. */
#if 0
