...
// FERRITE PATCH: upstream defines LOG_LEVEL LOG_INFO here, which
// causes monitor_init to chat 3 lines per construction over stderr
// ("Block size = ...", "Subblock size = ...", "N_FFT = ..."). Leaving
// LOG_LEVEL undefined makes ft8/debug.h's LOG() macro a no-op (see
// `#ifdef LOG_LEVEL` branch in debug.h). When debugging the C path
// flip this back on temporarily; the ferrited tracing layer doesn't
// need the chatter.
